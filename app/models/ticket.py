...
class TicketModel(TimestampMixin, IdInt, Base):
  __tablename__ = "Tickets"
  
  code = Column( String(250), index=True)
  user_id = Column(type_id_int, ForeignKey('Users.id'), nullable=False, index=True)
  reply = Column(Enum('create', 'question', 'answered', 'close', name='enum_ticket'), default='question', nullable=False)
  subject = Column( String(250), nullable=False)
  message = Column( Text, nullable=False)
  parent_id = Column(type_id_int, ForeignKey('Tickets.id'), index=True)

  # Relations
  user = relationship(UserModel, backref=backref("ticket", lazy='dynamic'))
  # user = relationship("UserModel", backref="ticket")
  parent = relationship( "TicketModel", lazy="joined", uselist=False, backref=backref("ticket", cascade="all, delete-orphan"), remote_side="TicketModel.id" )


  def __init__(self, **kwargs):
    super(TicketModel, self).__init__(**kwargs)
